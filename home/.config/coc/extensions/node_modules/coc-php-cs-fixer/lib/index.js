...
async function doFormat(context, outputChannel, document2, range) {
  ...
  const extensionConfig = import_coc.workspace.getConfiguration("php-cs-fixer");
  ...
  if (!toolPath) {
    if (import_fs.default.existsSync(import_path.default.join("vendor", "bin", "php-cs-fixer"))) {
      toolPath = import_path.default.join("vendor", "bin", "php-cs-fixer");
    } else if (import_fs.default.existsSync(import_path.default.join(context.storagePath, "php-cs-fixer"))) {
      toolPath = import_path.default.join(context.storagePath, "php-cs-fixer");
    } else {
  ...
  if (fixerConfig) {
    if (!import_path.default.isAbsolute(fixerConfig)) {
      let currentPath = opts.cwd;
      const triedPaths = [currentPath];
      while (!import_fs.default.existsSync(currentPath + import_path.default.sep + fixerConfig)) {
        const lastPath = currentPath;
        currentPath = import_path.default.dirname(currentPath);
        if (lastPath == currentPath) {
          import_coc.window.showErrorMessage(`Unable to find ${fixerConfig} file in ${triedPaths.join(", ")}`);
          return "";
        } else {
          triedPaths.push(currentPath);
        }
      }
      fixerConfig = currentPath + import_path.default.sep + fixerConfig;
    }
  ...
  return new Promise(function(resolve) {
    import_child_process.default.execFile("php", [...args, tmpFile.name], opts, function(err) {
      if (err) {
        ...
        if (err.code === "ENOENT") {
          import_coc.window.showErrorMessage("Unable to find the php-cs-fixer tool.");
          throw err;
        }
        import_coc.window.showErrorMessage("There was an error while running php-cs-fixer. Check the Developer Tools console for more information.");
        throw err;
      }
...
async function download(context, downloadMajorVersion) {
  ...
  const _path = import_path2.default.join(context.storagePath, "php-cs-fixer");
  const randomHex = (0, import_crypto.randomBytes)(5).toString("hex");
  const tempFile = import_path2.default.join(context.storagePath, `php-cs-fixer-${randomHex}`);
...
async function activate(context) {
  const extensionConfig = import_coc3.workspace.getConfiguration("php-cs-fixer");
  ...
  const outputChannel = import_coc3.window.createOutputChannel("php-cs-fixer");
  ...
  let toolPath = extensionConfig.get("toolPath", "");
  if (!toolPath) {
    if (import_fs3.default.existsSync(import_path3.default.join("vendor", "bin", "php-cs-fixer"))) {
      toolPath = import_path3.default.join("vendor", "bin", "php-cs-fixer");
    } else if (import_fs3.default.existsSync(import_path3.default.join(context.storagePath, "php-cs-fixer"))) {
      toolPath = import_path3.default.join(context.storagePath, "php-cs-fixer");
    }
  }
  if (!toolPath) {
  ...
  registerFormatter();
  context.subscriptions.push(import_coc3.commands.registerCommand("php-cs-fixer.fix", async () => {
  ...
  context.subscriptions.push(import_coc3.commands.registerCommand("php-cs-fixer.download", async () => {
    await downloadWrapper(context, downloadMajorVersion);
  }));
  if (isEnableActionProvider) {
    context.subscriptions.push(import_coc3.languages.registerCodeActionProvider(languageSelector, actionProvider, "php-cs-fixer"));
  }
}
